...
try {
    ...
    // Test avec une date spécifique pour voir les données d'aujourd'hui
    echo "\n🔍 Test avec les données d'aujourd'hui...\n";
    $today = date('Y-m-d');
    echo "📅 Date d'aujourd'hui: $today\n";
    
    // Récupérer les pointages d'aujourd'hui avec une plage horaire d'examen
    $stmt = $pdo->prepare("
        SELECT user_id, bsevtdt, devnm 
        FROM punchlog 
        WHERE CAST(bsevtdt AS DATE) = ? 
        AND CAST(bsevtdt AS TIME) BETWEEN '08:00:00' AND '12:00:00'
    ...
    ");
    $stmt->execute([$today]);
    $todayPunches = $stmt->fetchAll(PDO::FETCH_ASSOC);
    
    echo "📊 Pointages d'aujourd'hui (08:00-12:00):\n";
    foreach ($todayPunches as $punch) {
        echo "  - User ID: {$punch['user_id']}, Heure: {$punch['bsevtdt']}, Appareil: {$punch['devnm']}\n";
    }
